...
- LTF 截图时间为 `13:13:01`，HTF 截图时间为 `13:13:11`，两张截图相隔 10 秒。
- 较晚截图的实时报价约为 `64080`，作为本次最新价格参考。
- 图表标题栏 OHLC 可能属于十字光标选中的历史 K 线，不作为当前实时 OHLC。

### 当前多周期状态

| 周期 | Context / DVCA | 最新信号 | 当前解释 |
| --- | --- | --- | --- |
| 1H | FLAT / B95 / Trig 65589.7 | A-L HMR95 | 由 UP 降为 FLAT，正在测试 64170 与 63996 一带支撑 |
| 30m | FLAT / B100 / Trig 64438.9 | A-S HMR95 | 价格跌至全部可见均线下方，出现空头破位，但 Context 尚未转 DN |
| 15m | FLAT / B95 / Trig 65084.6 | L-ZONE HMR95 | 放量下跌，观察区未形成正式反转确认 |
| 5m | DN / S100 / Trig 64380.0 | TC-S | 空头执行周期确认，但 RSI 已进入低位 |
| 1m | DN / B invalid | TC-S | 空头延续，同时处于明显超卖位置 |

### 当前可见指标

| 周期 | MACD / Signal / Histogram | RSI / MA |
| --- | --- | --- |
| 1H | 40.9 / 7.8 / -33.1 | 36.65 / 51.94 |
| 30m | -9.0 / 33.5 / -42.6 | 37.08 / 53.31 |
| 15m | -35.0 / 24.6 / -59.5 | 34.40 / 50.25 |
| 5m | -106.5 / -54.3 / -52.2 | 22.43 / 40.84 |
| 1m | -103.6 / -75.1 / -28.5 | 13.70 / 25.88 |

### 当前可见均线与参考位

- 1H：`64530.1 / 64423.1 / 64264.4 / 63995.9`；主要水平位 `64170.0`。
- 30m：`64582.3 / 64547.4 / 64432.3 / 64266.3`。
- 15m：`64649.9 / 64630.4 / 64579.4 / 64445.6`；手工支撑 `64013.5`。
- 5m：`64666.6 / 64658.7 / 64618.6`；参考位 `64579.4`。
- 1m：`64671.9 / 64617.1 / 64531.0 / 64402.3`。

### 状态路径

```text
09:54: 1H UP / 30m FLAT / 15m FLAT / 5m FLAT / 1m DN
-> 64539-64584 收复失败
-> 1m TC-S 延续
-> 跌破 64439-64406
-> 5m DN / S100 / TC-S
...
-> 1H UP-to-FLAT
-> 测试 64013-63996
-> Multi-Level Support Failure
```

### 关键价位

- 已失守支撑：`64439-64406`、`64239-64170`。
- 压力：`64170-64266`、`64380-64445`、`64530-64582`、`64618-64672`。
- 支撑：`64013-63996`、`63900-63800`、`63650-63550`、`63200-63000`。

### 执行边界

- 当前方向偏空，但 1m RSI `13.70`、5m RSI `22.43` 表明空头位置已经延伸，不在约 64080 直接追空。
- 优先空头观察：反弹至 `64170-64266` 后出现空头拒绝。
- 更高质量空头观察：反弹进入 `64380-64445` 后出现空头拒绝。
- 延续空头观察：跌破 `63996`，随后回测 `64013` 失败。
- 1m 与 5m 保持 DN 时不执行多头；逆势多至少需要 `63996` 守住、1m 脱离 DN，并出现 TC-L 或新的 L-ZONE 观察链。
- 收复 `64266` 仅代表初步改善；收复 `64445` 才进入结构修复观察。

### 分支结论

- 30m Bullish Divergence：`Outcome Completed`；视觉背离方向跟随后已经完成验证，漏报问题继续作为独立回归测试项。
- Outcome11 High-Volatility Pullback：`Outcome11BearishBreakdown`。
- New Multi-Level Support Failure：`Outcome12Active`。
- 当前结论：`Direction Bearish / Location Oversold / Wait for Rebound Failure or Break-Retest`。
- 主案例仍保持 `Outcome20Recorded / Review20 / Outcome50 / Unknown`，不提前给出最终评级或最终结果。
